...
git clone https://github.com/freedesktop/xorg-libXv.git libXv_tmp
cd libXv_tmp
...
sh autogen.sh --prefix=$SYSROOT_PREFIX/usr --host=$TARGET_NAME --includedir=$SYSROOT_PREFIX/usr/include --disable-malloc0returnsnull
make install
cd ..
rm -fr ffmpeg_tmp
